...
build-docker-env:
	docker build -t localhost:5000/maven devops/maven
...
deploy-default:
	...
	scp target/*.jar jenkins@localhost:deploy/release
	- ssh jenkins@localhost 'kill `cat deploy/release/run.pid`'
	ssh jenkins@localhost 'java -jar deploy/release/*.jar > /dev/null 2>&1 & echo $$! > "deploy/release/run.pid"'
